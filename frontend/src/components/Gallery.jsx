...
const Gallery = () => {
  ...
  const deleteImage = async (id, location) => {
    try {
      ...
      setImages(prev =>
        prev.filter(img => !(img.id === id && img.locations.includes(location)))
      );
  ...
  return (
    <section style={styles.section}>
                ...
                {isAdmin && activeImage === img.id && (
                  <div style={styles.deleteButtonsContainer}>
                    ...
                    <button
                      ...
                      onClick={e => {
                        e.stopPropagation();
                        handleDeleteClick(img.id, "gallery");
                      }}
                    >
                      Delete
                    </button>
                    ...
                    {img.locations.includes("carousel") && (
                      <button
                        ...
                        onClick={e => {
                          e.stopPropagation();
                          handleDeleteClick(img.id, "carousel");
                        }}
                      >
                        Remove from Carousel
                      </button>
      ...
      <div style={styles.galleryContainer}>
        <div style={styles.imagesWrapper}>
          {carouselImages.length === 0 ? (
            <div>No carousel images found.</div>
          ) : (
            carouselImages.map(img => (
              <div
                key={img.id}
