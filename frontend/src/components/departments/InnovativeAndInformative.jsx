...
export default function InnovativeAndInfomative() {
  const [carouselImages, setCarouselImages] = useState([]);
  const [index, setIndex] = useState(0);
  const [originalImages, setOriginalImages] = useState([]);
  const [activeImage, setActiveImage] = useState(null);
...
const infoModalRef = useRef(null);
  ...
  try {
    const res = await axios.get(
      `${BASE_URL}/department-images?department=innovativeandinformative&type=carousel`
    );
  ...
  const fetchContributions = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/contributions?department=innovativeandinformative`);
 ...
 const handleDeleteImage = (img) => {
  ...
  const confirmDeleteImage = async () => {
    try {
      ...
      await axios.delete(`${BASE_URL}/department-wise/delete-image/${deleteModalImage.id}?department=innovativeandinformative&type=carousel`, {
  ...
  try {
    ...
    await axios.delete(
      `${BASE_URL}/contributions/delete/${deleteContributionId}?department=innovativeandinformative`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
  ...
  const handleAddImages = async () => {
    try {
      ...
      formData.append("department", "innovativeandinformative");
      formData.append("type", "carousel");
...
const deleteCarouselImage = async () => {
  try {
    ...
    await axios.delete(
      `${BASE_URL}/department-wise/delete-image/${deleteModalImage.id}?department=innovativeandinformative&type=carousel`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
const token = await user.getIdToken();
    try {
      await axios.delete(`${BASE_URL}/contributions/delete/${id}?department=innovativeandinformative`, {
  headers: {
    Authorization: `Bearer ${token}`,
  },
});
