...
// import img3 from "../../assets/outsidecharity4.jpeg";
...
const infoModalRef = useRef(null);
  ...
  try {
    const res = await axios.get(
      `${BASE_URL}/department-images?department=outsidecharity&type=carousel`
    );
  ...
  const fetchContributions = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/contributions?department=outsidecharity`);
 ...
 const handleDeleteImage = (img) => {
  ...
  const confirmDeleteImage = async () => {
    try {
      ...
      await axios.delete(`${BASE_URL}/department-wise/delete-image/${deleteModalImage.id}?department=outsidecharity&type=carousel`, {
  ...
  try {
    ...
    await axios.delete(
      `${BASE_URL}/contributions/delete/${deleteContributionId}?department=outsidecharity`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
  ...
  const handleAddImages = async () => {
    try {
      ...
      formData.append("department", "outsidecharity");
...
const deleteCarouselImage = async () => {
  try {
    ...
    await axios.delete(
      `${BASE_URL}/department-wise/delete-image/${deleteModalImage.id}?department=outsidecharity&type=carousel`,
      {
        headers: {
...
const token = await user.getIdToken();
    try {
      await axios.delete(`${BASE_URL}/contributions/delete/${id}?department=outsidecharity`, {
  headers: {
    Authorization: `Bearer ${token}`,
  },
});
